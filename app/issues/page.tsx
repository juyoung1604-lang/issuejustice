...
function IssuesContent() {
  ...
  useEffect(() => {
    const onPop = () => {
      const id = Number(new URLSearchParams(window.location.search).get('issue'))
      if (id && ISSUES.some(i => i.id === id)) setActiveModalId(id)
  ...
  if (sortParam === 'latest') displayed.sort((a, b) => b.date.localeCompare(a.date))
  else displayed.sort((a, b) => b.support - a.support)
  if (tagFilter !== "전체") displayed = displayed.filter(i => i.tags.includes(tagFilter))
  if (statusFilter !== "전체") displayed = displayed.filter(i => i.status === statusFilter)
  if (searchQuery.trim()) {
    const q = searchQuery.trim().toLowerCase()
    displayed = displayed.filter(i => i.title.toLowerCase().includes(q) || i.summary.toLowerCase().includes(q))
  }
  // 날짜 범위 필터 (YYYY.MM.DD 형식을 YYYY-MM-DD로 변환하여 비교)
  const toIso = (d: string) => d.replace(/\./g, '-')
  if (dateFrom) displayed = displayed.filter(i => toIso(i.date) >= dateFrom)
  if (dateTo) displayed = displayed.filter(i => toIso(i.date) <= dateTo)
  ...
  const activeNav = (href: string) => {
    if (href === '/ranking' || href === '/guide') return false
      ...
      <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
        ...
        <div className="max-w-7xl mx-auto px-6 h-16 md:h-20 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <Link
              href="/#register"
              className="hidden sm:inline-flex items-center gap-2 px-5 py-2.5 bg-red-500 text-white font-black text-sm rounded-xl hover:bg-red-600 hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300"
            >
            ...
            <button
              ...
              className="md:hidden w-9 h-9 flex items-center justify-center rounded-xl bg-gray-100 text-gray-700 hover:bg-gray-200 transition-colors"
            >
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden bg-white border-t border-gray-100 px-6 py-4 space-y-1">
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className={`text-[10px] font-medium ${activeNav(menu.href) ? 'text-gray-400' : 'text-gray-400'}`}>{menu.desc}</p>
                  </div>
                </div>
            ...
            <Link
              href="/#register"
              onClick={() => setIsMobileMenuOpen(false)}
              className="flex items-center justify-center gap-2 mt-3 px-4 py-3 bg-red-500 text-white font-black text-sm rounded-xl"
            >
          ...
          <div className="flex items-center gap-0 overflow-x-auto no-scrollbar">
            {NAV_MENUS.map(menu => (
              <Link
                key={menu.href}
                href={menu.href}
                className={`flex items-center gap-2 px-5 py-4 text-sm font-black whitespace-nowrap border-b-2 transition-all duration-200 ${
                  activeNav(menu.href)
      ...
      <main className="pt-[7.5rem] md:pt-36 pb-32 px-6">
        <div className="max-w-7xl mx-auto">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
              <div className="space-y-3">
                ...
                <p className="text-base md:text-lg text-gray-500 font-medium max-w-lg leading-relaxed">
                  {isLatestMode
                    ? '최근 접수된 순으로 정렬된 이슈 목록입니다.'
                    : '시민들이 직접 기록한 법집행 불균형 사례 전체를 확인하세요.'}
                </p>
              </div>
              ...
              <div className="flex gap-8 md:gap-10 shrink-0">
                {[
                  { label: "전체 이슈", value: ISSUES.length },
                  { label: "공론화 진행", value: ISSUES.filter(i => i.status === '공론화진행').length },
                  { label: "기관 전달", value: ISSUES.filter(i => i.status === '기관전달').length },
                ].map(stat => (
          ...
          <div className="flex flex-col sm:flex-row gap-3 mb-4">
            ...
            <div className="flex gap-2">
              <button
                ...
                className={`inline-flex items-center gap-1.5 px-4 py-3 rounded-2xl text-sm font-black transition-all duration-200 ${
                  !searchQuery && tagFilter === '전체' && statusFilter === '전체' && !dateFrom && !dateTo
                    ? 'bg-gray-900 text-white'
              ...
              <Link
                href="/issues?sort=latest"
                className={`inline-flex items-center gap-1.5 px-4 py-3 rounded-2xl text-sm font-black transition-all duration-200 ${
                  isLatestMode ? 'bg-gray-900 text-white' : 'bg-white border border-gray-200 text-gray-500 hover:border-gray-400'
                }`}
              >
          ...
          <div className="flex flex-wrap items-center gap-2 mb-6 p-4 bg-white border border-gray-100 rounded-2xl">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <input
                ...
                className="px-3 py-2 bg-gray-50 border border-gray-200 rounded-xl text-xs font-medium text-gray-600 focus:outline-none focus:border-red-400 focus:ring-2 focus:ring-red-500/10 transition-all cursor-pointer"
              />
              ...
              {(dateFrom || dateTo) && (
                <button
                  ...
                  className="inline-flex items-center gap-1 px-3 py-2 rounded-xl text-xs font-black text-gray-400 hover:text-red-500 hover:bg-red-50 transition-all"
                >
          ...
          <div className="flex flex-wrap gap-2 mb-10">
            {STATUS_FILTERS.map(s => (
              <button
                ...
                className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[11px] font-black transition-all duration-200 ${
                  statusFilter === s
            ...
            <span className="ml-2 inline-flex items-center text-xs font-bold text-gray-400">{displayed.length}건</span>
          </div>
              ...
              {displayed.map((issue, idx) => (
                ...
                >
                  ...
                  <div className="flex flex-wrap gap-1.5 mb-5">
                    {issue.tags.map(tag => (
                      <span key={tag} className="px-2 py-0.5 rounded-md text-[10px] font-bold bg-gray-50 text-gray-400 border border-gray-100">
                        {tag}
                      </span>
                    ))}
                  </div>
          ...
          <div className="mt-20 text-center space-y-4">
            ...
            <Link
              href="/#register"
              className="inline-flex items-center gap-3 px-10 py-5 bg-gray-900 text-white font-black rounded-2xl hover:bg-red-500 hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 group text-base md:text-lg"
            >
              직접 이슈 제보하기
        ...
        >
          ...
          >
            ...
            <div className="sticky top-0 bg-white/95 backdrop-blur-md border-b border-gray-50 px-8 md:px-10 py-6 md:py-8 flex items-start justify-between gap-4">
              ...
              <button
                onClick={closeModal}
                className="shrink-0 w-10 h-10 md:w-12 md:h-12 flex items-center justify-center bg-gray-50 rounded-2xl hover:bg-red-50 hover:text-red-500 transition-all duration-300"
              >
            ...
            <div className="overflow-y-auto flex-grow p-8 md:p-10 space-y-10 no-scrollbar">
              ...
              <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-[2rem] p-8 md:p-10 flex flex-col md:flex-row items-center justify-between gap-6 text-white">
                ...
                <button
                  ...
                  className={`px-8 py-4 md:px-10 md:py-5 rounded-2xl font-black text-base md:text-lg transition-all duration-500 active:scale-95 ${
                    supportedIds.includes(currentModalIssue.id)
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {currentModalIssue.attachments.map((raw, i) => {
                      ...
                      return (
                        <button
                          key={i}
                          onClick={() => { setActiveAttachment(att); setIsAttachmentViewerOpen(true) }}
                          className="w-full text-left flex items-center gap-4 p-4 bg-gray-50 border border-gray-100 rounded-2xl hover:border-red-200 hover:bg-red-50/30 transition-all group"
                        >
